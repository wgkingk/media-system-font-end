<template>
	<view class="content">
		...
		<scroll-view class="scroll" scroll-y>
			<view class="scroll-content">
				<view class="introduce-section">
					<text class="title">{{article.postTitle}}</text>
					<view class="introduce">
						<text>{{article.userEmail}}</text>
						<text>{{article.postClicks}}阅读</text>
						<text>{{article.createdTime}}</text>
					</view>
					
					<rich-text :nodes="article.postContent"></rich-text>
					<!-- 
				...
				<view class="container" v-show="loading === false">
					...
					<view class="evalution">
						<view  v-for="(item, index) in comments.list" :key="index"
							class="eva-item"
						>
							<image :src="item.userInfo.personalImage" mode="aspectFill"></image>
					...
					<uni-pagination title="标题文字" show-icon="true" :total="comments.totalPage * comments.pageSize" :pageSize="comments.pageSize" v-on:current="comments.startPage" @change="handleCurrentChange"></uni-pagination>
				</view>
				<!-- 加载图标 -->
		...
		<view class="bottom">
			...
			<text class="confirm-btn" @tap="handleCommentSubmit">提交</text>
		</view>
	</view>
</template>
		...
		methods: {
			...
			loadEvaList(){
				...
				request({
					...
					method: 'POST',
					data: {
						startPage: this.comments.startPage,
					...
					if (res.data.code === 1000) {
						...
						this.comments.list.forEach( item => {
			...
			handleCurrentChange: function(e) {
				this.comments.startPage = e.current
				this.loadEvaList()
			},
